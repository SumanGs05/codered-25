...
# Microphone array geometry (in meters) - CORRECTED HEXAGON
# 40mm radius from center, standard hexagon layout
MIC_SPACING = 0.040  # 40mm radius
MIC_POSITIONS = np.array([
    [0.000, 0.040, 0.0],   # Ch0: 90° (Top)
    [0.035, 0.020, 0.0],   # Ch1: 30° (Right-Top)
    [0.035, -0.020, 0.0],  # Ch2: 330° (Right-Bottom)
    [0.000, -0.040, 0.0],  # Ch3: 270° (Bottom)
    [-0.035, -0.020, 0.0], # Ch4: 210° (Left-Bottom)
    [-0.035, 0.020, 0.0],  # Ch5: 150° (Left-Top)
    [0.000, 0.000, 0.0],   # Ch6: Center
])
...
class FrequencyDomainBeamformer:
    ...
    def steering_vector(self, azimuth_deg, freq_hz, elevation_deg=0):
        ...
        Args:
            azimuth_deg: Target azimuth angle (0° = front, 90° = right)
...
def read_audio_stream(ser, beamformer):
    ...
    global running, stats, audio_output, current_zoom_angle
    ...
    while running:
        try:
            ...
            # Apply beamforming
            azimuth = map_zoom_to_azimuth(current_zoom_angle)
            output_audio = beamformer.process_chunk(audio_data, azimuth)
...
def map_zoom_to_azimuth(zoom_angle):
    """Map zoom FOV angle to beamforming azimuth"""
    return 0.0  # Point forward

def zoom_control_thread():
    """Thread to update zoom angle"""
    global current_zoom_angle, running
    
    print("Zoom control thread started")
...
def stats_thread():
    ...
    while running:
        ...
        print(f"\r[STATS] Packets: {packets} | Rate: {pps:.1f} pkt/s | "
              f"Errors: {errors} | Zoom: {current_zoom_angle:.1f}° | "
...
def main():
    ...
    print("=" * 70)
    print("FREQUENCY-DOMAIN Audio Beamforming - Raspberry Pi 4B")
    print("=" * 70)
    print(f"\nConfiguration:")
    print(f"  Sample Rate: {SAMPLE_RATE} Hz")
    print(f"  Channels: {CHANNELS} (6 hexagon + 1 center)")
    print(f"  FFT Size: {NFFT}, Hop: {HOP_LENGTH}")
    print(f"  Beamformer: {BEAMFORMER_TYPE}")
    print(f"  Mic Spacing: {MIC_SPACING*1000:.1f} mm")
    ...
    zoom_thread = threading.Thread(target=zoom_control_thread, daemon=True)
